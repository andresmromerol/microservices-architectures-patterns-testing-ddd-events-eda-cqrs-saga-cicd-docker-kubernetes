...
class RefreshTokenAuthRestControllerTest {
  ...
  @InjectMocks private RefreshTokenAuthRestController controller;
  ...
  void refreshToken_WithValidToken_ShouldReturnNewTokens() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/tokens/refresh-token")
                .header(
                    AuthTitleEnum.AUTHORIZATION_HEADER.getValue(), "Bearer " + VALID_REFRESH_TOKEN)
  ...
  void refreshToken_WithMissingToken_ShouldReturnBadRequest() throws Exception {
    mockMvc
        .perform(post("/api/v1/tokens/refresh-token").contentType(MediaType.APPLICATION_JSON))
  ...
  void refreshToken_WithExpiredToken_ShouldReturnUnauthorized() throws Exception {
    ...
    doThrow(new UserAuthTokenExpiredException())
        .when(commandBus)
        .dispatch(any(ValidateTokenCmd.class));
    mockMvc
        .perform(
            post("/api/v1/tokens/refresh-token")
